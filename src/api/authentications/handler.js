...
class AuthenticationsHandler {
  ...
  async postAuthenticationHandler(request, h) {
    const { username, password } = this._validator.validatePostAuthenticationPayload(request.payload);
    const { id: userId, password: hashedPassword } = await this._usersService.getUserByUsername(username);
